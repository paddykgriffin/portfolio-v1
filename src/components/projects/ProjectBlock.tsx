...
const ProjectBlock = ({ name, id, logo, thumbnail, tags }: Project) => {
  return (
    <div className="grid text-center group overflow-hidden">
      <a href={`/projects/${id}`} className="col-start-1 row-start-1 content-center z-20">
        <div className="relative h-1/2">
          ...
          <div className="col-start-1 row-start-1 tags flex text-white opacity-0 justify-center absolute left-1/2 -translate-x-1/2 top-1/2 group-hover:opacity-100">
            <LuTag className="text-4xl text-white w-5 h-5 mr-2" /> {tags.join(', ')}
          </div>

          <Button
            variant="outline"
            className="opacity-0  text-white text-xl font-normal py-5 px-24 hover:bg-white hover:text-black absolute top-[50%] left-1/2 -translate-x-1/2 -translate-y-1/2 group-hover:translate-y-5/6 transition-top duration-1000 transition-transform group-hover:opacity-100"
          >
            view project
          </Button>
        </div>
